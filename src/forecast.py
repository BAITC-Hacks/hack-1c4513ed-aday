...
def forecast(demand, season, config=DEFAULT, horizon=6):
    ...
    complete = demand[demand.month < pd.Timestamp(config.as_of).replace(day=1)][["code", "month", "stable_qty"]].copy()
    if complete.empty:
    ...
    complete["positive"] = (complete.stable_qty > 0).astype(int)
    stats = complete.groupby("code").agg(n=("month", "size"), positive=("positive", "sum"), total=("stable_qty", "sum"), avg=("stable_qty", "mean"))
    stats["article_season"] = (stats.n >= 18) & (stats.positive >= 12) & (stats.total >= 60) & (stats.avg > 0)
    month_mean = complete.groupby(["code", "month_num"]).stable_qty.mean().rename("month_mean")
    ...
    article_index = (levels.month_mean / levels.avg.replace(0, np.nan)).fillna(levels.brand_index)
    levels["season_index"] = np.where(levels.article_season, .5 * article_index + .5 * levels.brand_index, levels.brand_index)
    scale = levels.groupby("code").season_index.transform("mean")
    levels["season_index"] = (levels.season_index / scale.replace(0, 1)).clip(.5, 2)
    ...
    complete["deseason"] = complete.stable_qty / complete.season_index.clip(lower=.2)
    ...
    current = tail.groupby("code").agg(current_sum=("stable_qty", "sum"), current_n=("month", "size"))
    previous = tail[["code", "month"]].copy()
    previous["month"] = previous.month - pd.DateOffset(years=1)
    previous = previous.merge(complete[["code", "month", "stable_qty"]], on=["code", "month"], how="left")
    earlier = previous.groupby("code").agg(earlier_sum=("stable_qty", "sum"), earlier_n=("stable_qty", "count"))
