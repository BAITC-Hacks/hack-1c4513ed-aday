...
def restore(cleaned, stocks, season):
    ...
    available = data[(data.stock > 0) & (data.month < pd.Timestamp(DEFAULT.as_of).replace(day=1))].copy()
    ...
    data["stockout"] = (data.month < pd.Timestamp(DEFAULT.as_of).replace(day=1)) & (data.stock <= 0) & ((data.next_stock <= 0) | low_sales) & (data.typical > 0)
