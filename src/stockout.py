...
def restore(cleaned, stocks, season, config=DEFAULT):
    ...
    available = data[(data.stock > 0) & (data.month < pd.Timestamp(config.as_of).replace(day=1))].copy()
    ...
    data["stockout"] = (data.month < pd.Timestamp(config.as_of).replace(day=1)) & (data.stock <= 0) & ((data.next_stock <= 0) | low_sales) & (data.typical > 0)
